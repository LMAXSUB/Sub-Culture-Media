/* ============================================================
   ...
   Public API (used by downloads.html's Play buttons):
     window.ScPlayer.playTrackByFile(file) — jumps to and plays
     that track, re-opening the bar if it was closed.
   ============================================================ */
(function () {
  "use strict";
  ...
  function build(tracks) {
    ...
    // Public hook: lets other pages (like the Downloads grid) jump straight
    // to and play a specific track by its file path, and reveal the bar
    // again if the visitor had closed it.
    window.ScPlayer = {
      playTrackByFile: function (file) {
        var i = tracks.findIndex(function (t) { return t.file === file; });
        if (i === -1) return false;
        bar.classList.remove("scp-hidden");
        document.body.classList.add("scp-padded");
        launch.style.display = "none";
